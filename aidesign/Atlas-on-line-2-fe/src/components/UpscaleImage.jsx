...
const UpscaleImage = () => {
  ...
  const handleFileChange = (e) => {
    ...
    setFileName(file.name) // Preview the uploaded image
...
const handleFileRemove = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://212.64.220.94:3000/upscale-image', formData, {
        headers: {
      ...
      setUpscaledImage(response.data.data[0]?.url); // Assuming API returns `upscaledImageUrl`
      setLoading(false);
    } catch (err) {
